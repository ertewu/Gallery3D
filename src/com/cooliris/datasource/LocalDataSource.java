...
import com.cooliris.cache.CacheConstants;
...
public class LocalDataSource implements DataSource {
    ...
    public LocalDataSource(final Context context, final String uri, final boolean flattenAllItems) {
        ...
        // 12-17 19:47:14.762: I/ertewu(7767): localDataSource
        // r78:content://media/external/images/media|null
    ...
    public void loadItemsForSet(MediaFeed feed, MediaSet parentSet, int rangeStart, int rangeEnd) {
        ...
        if (mSingleUri && !mDone) {
            ...
            if (item != null) {
                ...
                long dateTaken = CacheHelper.fetchDateTaken(item);
                if (dateTaken != -1L) {
            ...
            String where = null;
            try {
                Cursor cursor = cr.query(uriImages, CacheConstants.PROJECTION_IMAGES, where, null, null);
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    do {
                        ...
                        CacheHelper.populateMediaItemFromCursor(item, cr, cursor, CacheConstants.BASE_CONTENT_STRING_IMAGES);
                        feed.addItemToMediaSet(item, parentSet);
    ...
    public static MediaItem createMediaItemFromUri(Context context, Uri target, int mediaType) {
        ...
        try {
            ...
            final String[] projection = (mediaType == MediaItem.MEDIA_TYPE_IMAGE) ? CacheConstants.PROJECTION_IMAGES
                    : CacheConstants.PROJECTION_VIDEOS;
            Cursor cursor = cr.query(uri, projection, whereClause, null, null);
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    item = new MediaItem();
                    CacheHelper.populateMediaItemFromCursor(item, cr, cursor, uri.toString() + "/");
                    item.mId = id;
                }
                cursor.close();
                cursor = null;
    ...
    public static MediaItem createMediaItemFromFileUri(Context context, String fileUri) {
        ...
        String whereClause = Images.ImageColumns.BUCKET_ID + "=" + bucketId + " AND " + Images.ImageColumns.DATA + "='"
                + filepath + "'";
        try {
            Cursor cursor = cr.query(Images.Media.EXTERNAL_CONTENT_URI, CacheConstants.PROJECTION_IMAGES, whereClause,
                    null, null);
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    item = new MediaItem();
                    CacheHelper.populateMediaItemFromCursor(item, cr, cursor,
